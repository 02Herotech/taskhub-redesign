...
import listingZodSchema from "@/schema";
...
const EditListing = ({ showEditModal, setShowEditModal }: EditModalProps) => {
  ...
  const handleOnImageInputChange = ({
    ...
    if (uploadFile) {
      ...
      reader.onloadend = () => {
        ...
        setValue(imageFieldNames[index - 1] as "image1" | "image2" | "image3" | "image4", img);
      };
  ...
  return (
    <Popup isOpen={showEditModal} onClose={() => setShowEditModal(false)}>
      <main className="relative space-y-8 p-4 lg:p-8 max-h-[90vh] w-full overflow-y-auto">
        {showModal && (

...
const ProfileCompletion = ({ fetchedUserData }: ProfileCompletionType) => {
  ...
  return (
    <section className="flex min-h-64 flex-col items-center gap-7 rounded-lg bg-[#EBE9F4] p-4 md:grid md:grid-cols-12 md:gap-24">
      ...
      <div className="col-span-8 space-y-4">
        ...
        <div className="flex w-full flex-wrap items-center justify-center gap-4 md:w-10/12 md:justify-start">
          {profileProgressData.map((item, index) => (
            <Link
              href={item.status ? "#" : "/customer/profile/edit-profile"}
              key={index}

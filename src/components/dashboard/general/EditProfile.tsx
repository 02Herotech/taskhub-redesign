...
const EditProfile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (!token) return;
      try {
        const url = `https://api.oloja.com.au/api/v1/${isServiceProvider ? 'service_provider' : 'customer'}/profile`;
  ...
  useEffect(() => {
    const fetchLocationByPostcode = async () => {
      ...
      try {
        const url = `https://api.oloja.com.au/api/v1/util/locations/search?postcode=${watchPostcode}`;
        const { data } = await axios.get(url);
